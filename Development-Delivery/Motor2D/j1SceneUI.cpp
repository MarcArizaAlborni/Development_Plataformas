...
{
	BROFILER_CATEGORY("SceneUI Start()", Profiler::Color::LightGoldenRodYellow)
	bool ret = true;
...
{
	BROFILER_CATEGORY("SceneUI Update()", Profiler::Color::CornflowerBlue)
	...
	if (App->audio->Area1Level1 == true && OnMainMenu == false && Menu_Music==true ) {
		Mix_HaltMusic();
	...
	if (App->entityManager->player != NULL) {
		...
		if (HealthChanged == true) {
			...
			if (HealthFill != true) {
				if (HealthBoosted == true) {
					App->entityManager->player->Life_Num = App->entityManager->player->Life_Num + 1;

				}
			}
			else {
				App->entityManager->player->Life_Num = 5;
			}
		...
		if (HealthChanged == true) {
			if (HealthBoosted != true) {
				if (App->entityManager->player->Life_Num == 4) {
					PlayerUI_Ingame.add(lifes[0] = (App->gui->CreateSprite({ 5,10 }, { 458,80,110,30 }, true)));
					...
					PlayerUI_Ingame.add(lifes[1] = (App->gui->CreateSprite({ 5,10 }, { 458,111,110,30 }, true)));
					...
					PlayerUI_Ingame.add(lifes[2] = (App->gui->CreateSprite({ 5,10 }, { 458,144,110,30 }, true)));
					...
					PlayerUI_Ingame.add(lifes[3] = (App->gui->CreateSprite({ 5,10 }, { 458,175,110,30 }, true)));
					player_status[3] = (App->gui->CreateSprite({ 120,10 }, { 308,45,20,35 }, true));
					

				}
				if (App->entityManager->player->Life_Num == 5) {
					PlayerUI_Ingame.add(lifes[0] = (App->gui->CreateSprite({ 5,10 }, { 458,45,110,34 }, true)));
					player_status[0] = (App->gui->CreateSprite({ 120,10 }, { 413,45,20,35 }, true));
					//HealthChanged = false;
	...
	if (App->entityManager->player->Life_Num<=0) {
		Revive();
	}
				...
				case BOX_SLIDER1:
					...
					MusicSliderBox->pos.x = VolumeMusicBoxPos.x - 20;
					   ...
					   if ((VolumeMusicBoxPos.x < 447) && (VolumeMusicBoxPos.x >= 410)) {
						   ...
						   Mix_VolumeMusic(App->audio->VolumeMusicValue);
					   }
					   
					   if ((VolumeMusicBoxPos.x < 409) && (VolumeMusicBoxPos.x >= 380)) {
						   App->audio->VolumeMusicValue = 50;
						   Mix_VolumeMusic(App->audio->VolumeMusicValue);
					   }
		...
		while (MainMenuButtons != NULL) {
			if (MainMenuButtons->data->OnClick()) {
				...
				case START_PLAY:
					...
					App->entityManager->player->Life_Num = 4;
			...
			if (Console_Buttons3->data->OnClick()) {
				...
				case CONSOLE_GOD:
					if (ConsoleSecurity == true) {
						...
						App->entityManager->player->GOD_MODE = true;
						ConsoleSecurity = true;
...
{
	BROFILER_CATEGORY("SceneUI PostUpdate()", Profiler::Color::AntiqueWhite)
...
{
	BROFILER_CATEGORY("SceneUI Clean()", Profiler::Color::RosyBrown)
