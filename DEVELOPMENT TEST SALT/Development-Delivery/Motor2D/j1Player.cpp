...
j1Player::j1Player() 
{
 ...
 bool j1Player::Awake(pugi::xml_node& node)
 {
	 ...
	 Character_vel.x = node.attribute("velocityX").as_float();
	 Character_vel.y= node.attribute("velocityY").as_float();
...
bool j1Player::Update(float dt)
 {
	...
	float positionLOGX = CurrentPosition.x;
	float positionLOGY = CurrentPosition.y;
	LOG("POSITION --->    X = %f ,  Y =  %f", positionLOGX, positionLOGY);
 ...
 void j1Player::OnCollision(Collider* A, Collider* B) {
	 ...
	 if (A->type == ObjectType::Player && B->type == ObjectType::Platform) {
		 ...
		 //from above
		 else if (CurrentPosition.y + A->rect.h - Character_vel.y - 2 < B->rect.y && A->rect.x < B->rect.x + B->rect.w && A->rect.x + A->rect.w > B->rect.x) { // from above
